...
class Individual:
    def __init__(self, dna_size: int, maximize: bool) -> None:
        # inverse binary repr
        self.generate_random_dna(dna_size=dna_size)
        self.evaluate(maximize=maximize)

    def generate_random_dna(self, dna_size: int) -> None:
        self.dna = np.random.randint(0, 2, dna_size)
...
class GeneticAlgorithm:
    ...
    def create_population(
        size: int, bitstr_size: int, maximize: bool
    ) -> list[Individual]:
        return [
            Individual(bitstr_size=bitstr_size, maximize=maximize) for _ in range(size)
        ]
